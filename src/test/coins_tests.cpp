...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            // are tested.
            bool test_habuddiein_before = (insecure_rand() & 0x3) == 0; // TODO change to InsecureRandBits(2) when backporting Bitcoin #10321
            bool test_habuddiein_after = (insecure_rand() & 0x3) == 0; // TODO change to InsecureRandBits(2) when backporting Bitcoin #10321

            bool result_habuddiein = test_habuddiein_before ? stack.back()->HaveCoin(COutPoint(txid, 0)) : false;
            const Coin& entry = (insecure_rand() % 500 == 0) ? AccessByTxid(*stack.back(), txid) : stack.back()->AccessCoin(COutPoint(txid, 0));
            BOOST_CHECK(coin == entry);
            BOOST_CHECK(!test_habuddiein_before || result_habuddiein == !entry.IsSpent());

            if (test_habuddiein_after) {
