...
const char * const BITCOIN_CONF_FILENAME = "buddie.conf";
const char * const BITCOIN_PID_FILENAME = "buddied.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "buddie" is a composite category enabling all Buddie-related debug output
            if(ptrCategory->count(string("buddie"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BuddieCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BuddieCore
    // Mac: ~/Library/Application Support/BuddieCore
    // Unix: ~/.buddiecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BuddieCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BuddieCore";
#else
    // Unix
    return pathRet / ".buddiecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty buddie.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override buddie.conf
